        ...
        public string GenerateSignature(Uri url, string consumerKey, string consumerSecret, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce, SignatureTypes signatureType, out string normalizedUrl, out string normalizedRequestParameters)
        {
            ...
            switch (signatureType)
            {
                ...
                case SignatureTypes.HmacSha256:
                    string signatureBase1 = GenerateSignatureBase(url, consumerKey, token, tokenSecret, httpMethod, timeStamp, nonce, HMACSHA256SignatureType, out normalizedUrl, out normalizedRequestParameters);

                    var crypto = new HMACSHA256();
                    var key = string.Concat(consumerSecret, "&", tokenSecret);

                    crypto.Key = Encoding.ASCII.GetBytes(key);
                    return GenerateSignatureUsingHash(signatureBase1, crypto);
